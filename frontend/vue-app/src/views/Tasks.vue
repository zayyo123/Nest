  ...
  <section class="page-stack">
    ...
    <section class="panel">
      <div class="toolbar toolbar-wrap">
        <el-input v-model="filters.q" placeholder="搜索任务标题或描述" clearable @input="resetPage" />
      ...
      <el-table :data="pagedTasks" v-loading="loading" empty-text="暂无任务">
        ...
        <el-table-column label="项目" min-width="160">
          <template #default="{ row }">{{ row.project?.name || '未关联项目' }}</template>
        </el-table-column>
...
export default defineComponent({
  ...
  setup() {
    ...
    const pageSize = 8
    const filters = reactive<{ q: string; status: '' | TaskStatus; projectId: number | '' }>({
      q: '',
      status: '',
      projectId: '',
    })
    ...
    const editForm = reactive<{ title: string; description: string; status: TaskStatus; projectId: number | '' }>({
      title: '',
      description: '',
      status: 'TODO',
      projectId: '',
